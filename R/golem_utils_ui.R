...
histo_slider_input <- function(inputId, data, width = "600px", height = "400px") {
  # Manually convert the numeric vector to a JSON array string.
  # Example: c(1,2,3) becomes "[1,2,3]"
  ...
  tags$div(
    ...
    tags$div(id = paste0(inputId, "_histogram")),
    # Container for the slider. Width set to 100% to match the histogram.
    tags$div(id = paste0(inputId, "_slider"), style = "width:100%; margin:20px auto;"),

    # Include external JavaScript and CSS dependencies.
    tags$head(
      tags$script(src = "https://d3js.org/d3.v7.min.js"),
      tags$link(rel = "stylesheet", href = "https://cdnjs.cloudflare.com/ajax/libs/noUiSlider/15.7.0/nouislider.min.css"),
      tags$script(src = "https://cdnjs.cloudflare.com/ajax/libs/noUiSlider/15.7.0/nouislider.min.js")
    ),
    ...
    # JavaScript to build the histogram and slider widget.
    tags$script(HTML(sprintf("
      (function() {
        function createHistoSlider(containerId, data) {
          ...
          // Get the actual width of the histogram container.
          var containerW = histoDiv.clientWidth;
          if (!containerW || containerW <= 0) { containerW = 600; } // fallback
          ...
          // Append the SVG element for the histogram.
          ...
          // Create an x-scale based on the data's extent.
          var xExtent = d3.extent(data);
          var x = d3.scaleLinear().domain(xExtent).nice().range([0, width]);
          // Create bins; using a large number of thresholds (e.g., 100) to approximate continuity.
          var bins = d3.bin().domain(x.domain()).thresholds(x.ticks(100))(data);

          // Create a y-scale based on the frequency of bins.
          ...
          // Remove y-axis (not appended).

          // Draw a bar for each bin. Adjust the rectangle so that bars touch each other.
          ...
          bar.append('rect')
             ...
             .attr('fill', 'blue'); // Default color is blue.

          // Initialize the noUiSlider in the slider container, aligning its range with the histogram's x-axis.
          ...
          noUiSlider.create(sliderDiv, {
            ...
            tooltips: true,
            format: {
              to: function (value) { return parseFloat(value.toFixed(2)); },
              from: function (value) { return Number(value); }
            }
          });

          // Update bar colors based on the slider range.
          // Each bar's midpoint is computed; if it falls within the slider range, the bar turns red.
          function updateHighlights(range) {
            ...
            bar.select('rect')
              .attr('fill', function(d) {
                var mid = (d.x0 + d.x1) / 2;
                return (mid >= minVal && mid <= maxVal) ? 'red' : 'blue';
              });
          }

          // Set the initial highlight (full x-range).
          updateHighlights(xExtent);

          // When the slider updates, refresh the bar colors and send the range to Shiny.
          sliderDiv.noUiSlider.on('update', function (values) {
            var minVal = parseFloat(values[0]),
                maxVal = parseFloat(values[1]);
            updateHighlights([minVal, maxVal]);
            if (window.Shiny) {
              Shiny.setInputValue(containerId + '_range', {min: minVal, max: maxVal}, {priority: 'event'});
            }
          });
        }
...
histo_slider_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    update_data <- function(new_data) {
      ...
      session$sendCustomMessage(
        type = "histo_slider_update",  # Message type
        message = list(
          id = session$ns(id),  # Use namespaced ID
          data = data_json
        )
      )
    }
